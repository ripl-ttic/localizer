...
static void realloc_temp_weights(carmen3d_localize_particle_filter_p filter, int num_particles)
{
    int i;
...
carmen3d_localize_particle_filter_p carmen3d_localize_particle_filter_new(carmen3d_localize_param_p param)
{
    carmen3d_localize_particle_filter_p filter;

    /* allocate the particle filter */
    filter = (carmen3d_localize_particle_filter_p) calloc(1, sizeof(carmen3d_localize_particle_filter_t));
    ...
    filter->particles = (carmen3d_localize_particle_p) calloc(filter->param->num_particles,
                                                    ...
                                                    carmen_robot_laser_message *laser, carmen3d_localize_map_p map)
{
    ...
    //this initializes the weights - which is why the laser message is needed here
    for (i = 0; i < filter->param->global_test_samples; i++) {
        if (i % 10000 == 0) {
            fprintf(stderr, "\rDoing global localization... (%.1f%% complete)", i
                    / (float) filter->param->global_test_samples * 100.0);
        ...
        } while (map->carmen_map.map[(int) point.x][(int) point.y] > filter->param->occupied_prob
        ...
        for (j = 0; j < laser->num_readings && (queue->last == NULL || prob > queue->last->prob); j
    ...
    if (filter->param->do_scanmatching) {
        for (i = 0; i < filter->param->num_particles; i++) {
            ...
            carmen3d_localize_laser_scan_gd(laser->num_readings, laser->range, laser->config.angular_resolution,
                                            laser->config.start_angle, &point, filter->param->front_laser_offset, map, filter->param->laser_skip);
...
void carmen3d_localize_initialize_particles_gaussians(carmen3d_localize_particle_filter_p filter, int num_modes,
...
int carmen3d_localize_initialize_particles_placename(carmen3d_localize_particle_filter_p filter,
                                                     carmen_map_placelist_p placelist, char *placename)
{
    ...
    carmen3d_localize_initialize_particles_gaussian(filter, mean, std);
    return 0;
}

/* initialize particles from a gaussian distribution */

void carmen3d_localize_initialize_particles_gaussian(carmen3d_localize_particle_filter_p filter, carmen_point_t mean,
                                                     carmen_point_t std)
{
    carmen3d_localize_initialize_particles_gaussians(filter, 1, &mean, &std);
}

/* initialize particle positions and weights from parameters */

void carmen3d_localize_initialize_particles_manual(carmen3d_localize_particle_filter_p filter, double *x, double *y,
                                                   double *theta, double *weight, int num_particles)
{
    int i;

    if (num_particles != filter->param->num_particles) {
        filter->particles = (carmen3d_localize_particle_p) realloc(filter->particles, num_particles
                                                                   * sizeof(carmen3d_localize_particle_t));
        carmen_test_alloc(filter->particles);
...
void carmen3d_localize_incorporate_odometry(carmen3d_localize_particle_filter_p filter, carmen_point_t odometry_position)
{
    int i, backwards;
    double delta_t, delta_theta;
    double dx, dy, odom_theta;
#ifdef HOLONOMIC
    double dx_noise, dy_noise;
#endif
    ...
    if(delta_dist ==0 && delta.theta ==0){
        return;
    }
    double downrange_std_dev = HOLONOMIC_XY_DEV; //TODO: don't hardcode me!
    for (i = 0; i < filter->param->num_particles; i++) {
        carmen_point_t noisy_delta;
        noisy_delta.x = carmen_gaussian_random(delta.x, HOLONOMIC_XY_DEV);
        noisy_delta.y = carmen_gaussian_random(delta.y, HOLONOMIC_XY_DEV);
        noisy_delta.theta = carmen_gaussian_random(delta.theta, HOLONOMIC_THETA_DEV);
        carmen_point_t particle_pos={filter->particles[i].x,filter->particles[i].y,filter->particles[i].theta};
        carmen_point_t new_particle_pos = carmen3d_body2D_sum(&particle_pos, &noisy_delta);
        filter->particles[i].x=new_particle_pos.x;
        filter->particles[i].y=new_particle_pos.y;
        filter->particles[i].theta = new_particle_pos.theta;
    }
...
static int global_mode_test(carmen3d_localize_particle_filter_p filter)
{
    int i;
    ...
    for (i = 0; i < filter->param->num_particles; i++)
        if (fabs(filter->particles[i].x - mean_x) > filter->param->global_distance_threshold || fabs(filter->particles[i].y
                                                                                                     - mean_y) > filter->param->global_distance_threshold)
            return 1;
    return 0;
}
...
void carmen3d_localize_incorporate_laser(carmen3d_localize_particle_filter_p filter, carmen3d_localize_map_p map,
                                         int num_readings, float *range, double forward_offset, double angular_resolution, double laser_maxrange,
                                         double first_beam_angle, int backwards)
{
        ...
        for (i = 0; i < filter->param->num_particles; i++) {
            ...
            for (j = 0; j < num_readings; j += filter->param->laser_skip)
                if (filter->laser_mask[j]) {
                    ...
                    else if (filter->param->constrain_to_map && (robot_x < 0 || robot_y < 0 || robot_x >= map->config.x_size
                                                                 || robot_y >= map->config.y_size || map->carmen_map.map[robot_x][robot_y] > filter->param->occupied_prob))
                        filter->particles[i].weight += global_log_small_prob;
                    else
...
void carmen3d_localize_resample(carmen3d_localize_particle_filter_p filter)
{
    ...
    temp_particles
        = (carmen3d_localize_particle_p) calloc(filter->param->num_particles, sizeof(carmen3d_localize_particle_t));
    ...
    for (i = 0; i < filter->param->num_particles; i++) {
        ...
        while (position > cumulative_sum[which_particle])
            which_particle++;
        memcpy(temp_particles + i, filter->particles + which_particle, sizeof(carmen3d_localize_particle_t));
    }
...
void carmen3d_localize_run(carmen3d_localize_particle_filter_p filter, carmen3d_localize_map_p map,
                           carmen_robot_laser_message *laser, double forward_offset, int backwards)
{
...
void carmen3d_localize_laser_scan_gd(int num_readings, float *range, double angular_resolution, double first_beam_angle,
                                     carmen_point_p laser_pos, double forward_offset, carmen3d_localize_map_p map, int laser_skip)
{
...
void carmen3d_localize_summarize(carmen3d_localize_particle_filter_p filter, carmen3d_localize_summary_p summary,
                                 carmen3d_localize_map_p map, int num_readings, float *range, double angular_resolution, double first_beam_angle,
                                 double forward_offset, int backwards)
{
    ...
    if (filter->param->do_scanmatching)
        carmen3d_localize_laser_scan_gd(summary->num_readings, range, angular_resolution, first_beam_angle, &summary->mean,
